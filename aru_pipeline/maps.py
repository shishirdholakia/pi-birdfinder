...
def _unit_display_label(unit: str) -> str:
    word_digits = {
        "zero": "0",
        "one": "1",
        "two": "2",
    ...
    unit_s = str(unit)
    return word_digits.get(unit_s.lower(), unit_s)


def _station_pin_icon(unit: str, fill_color: str = "#1f77b4", text_color: str = "#ffffff"):
    import folium

    label = html.escape(_unit_display_label(unit))
    return folium.DivIcon(
        icon_size=(32, 42),
        icon_anchor=(16, 42),
        popup_anchor=(0, -38),
        class_name="aru-station-pin-wrapper",
        html=f"""
        <div style="position:relative;width:32px;height:42px;">
          <div style="
              position:absolute;left:2px;top:0;width:28px;height:28px;
              background:{fill_color};border:2px solid #ffffff;
              border-radius:50% 50% 50% 0;transform:rotate(-45deg);
              box-shadow:0 2px 6px rgba(0,0,0,0.45);">
            <div style="
                transform:rotate(45deg);width:28px;height:28px;line-height:28px;
                text-align:center;color:{text_color};font-size:14px;
...
def write_event_map(out_html: str, station_latlon: Dict[str, Tuple[float, float]], pos_xy: Dict[str, np.ndarray], ref_unit: str, source_xy: np.ndarray, tdoa_s: Dict[str, float], sound_speed_m_s: float, samples_xy: Optional[np.ndarray] = None, ellipse95_xy: Optional[np.ndarray] = None, loo_xy: Optional[Dict[str, np.ndarray]] = None) -> None:
    ...
    for u, ll in station_latlon.items():
        folium.Marker(ll, tooltip=u, popup=f"Station {u}<br>{ll[0]:.8f}, {ll[1]:.8f}", icon=_station_pin_icon(u), pane="locations").add_to(fg_st)
        folium.PolyLine([source_ll, ll], color="#ffffff", weight=2, opacity=0.6, pane="constraints").add_to(fg_st)
        features.append(_pt_feature(ll[0], ll[1], {"kind": "station", "unit": u, "label": _unit_display_label(u), "name": f"station {u}", "radius": 7}))
