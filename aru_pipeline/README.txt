...
aru_calibrate_session.py     Create or update session_calibration.txt from a known calibration clap.
aru_localize_event.py        Localize an unknown event using fetched clips and optional session_calibration.txt.
...
2. Create/update calibration. Default source is a clap at five:
   python aru_calibrate_session.py /path/to/event_2026-05-18_20-00-30 \
     --calibration-txt /path/to/session_calibration.txt
...
4. Localize it:
   python aru_localize_event.py /path/to/event_2026-05-18_21-13-42 \
     --calibration-txt /path/to/session_calibration.txt --mode impulse
...
Notes
-----
- session_calibration.txt is created/updated by the calibration script and consumed by the event localizer.
...
Suggested timestamp-first shell workflow:
  ...
  python aru_calibrate_session.py "$EVENT_DIR" --source-unit five --calibration-txt data/calibrations/session_calibration.txt

  EVENT_DIR=$(python acquire.py "2026-05-06 21:12:30" --clip-half-s 30 | awk -F' = ' '/^event_dir/{print $2}')
  python aru_localize_event.py "$EVENT_DIR" --calibration-txt data/calibrations/session_calibration.txt --mode impulse
